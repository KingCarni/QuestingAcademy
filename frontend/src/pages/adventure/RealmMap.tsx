        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_390px] xl:items-stretch">
          <div ref={canvasRef} onClick={handleCanvasClick} data-testid="realm-world-canvas" className="relative min-h-[720px] md:min-h-[820px] rounded-[2.75rem] overflow-hidden border-[7px] border-white shadow-2xl shadow-indigo-900/20 cursor-pointer select-none ring-4 ring-primary/10" style={{ backgroundImage: activeSceneImage ? `url(${activeSceneImage})` : undefined, backgroundSize: "cover", backgroundPosition: "center", backgroundColor: "#BFE0F2" }}>
            ...
            {!activeSceneImage && <>
              ...
              {live.map((r: any, i: number) => { const pos = LIVE_POSITIONS[i % LIVE_POSITIONS.length]; const selected = selectedRealm?.id === r.id; return <button type="button" key={r.id} onClick={(e) => { e.stopPropagation(); selectRealm(r.id, pos, false); }} onDoubleClick={(e) => { e.stopPropagation(); selectRealm(r.id, pos, true); }} data-testid={`realm-node-${r.id}`} className="group absolute -translate-x-1/2 -translate-y-1/2 focus:outline-none z-20" style={pos}><div className="relative flex flex-col items-center"><div className={`w-32 h-32 md:w-40 md:h-40 rounded-[40%] border-[5px] shadow-2xl grid place-items-center ${selected ? "border-gold bg-gradient-to-b from-[#FFF6D8] via-[#E8F4E1] to-[#86A789] ring-4 ring-gold/35" : "border-white bg-gradient-to-b from-[#E8F4E1] via-[#CDE0CF] to-[#86A789]"}`}><span className="text-6xl md:text-7xl drop-shadow-sm" aria-hidden>{emojiFor(r.biome)}</span></div><div className={`mt-3 chip border-white max-w-[210px] truncate text-center shadow-sm ${selected ? "bg-primary text-white" : "bg-white/95 text-ink"}`}><MapPin size={12} strokeWidth={3} /> <span className="h-bouncy">{r.name}</span></div></div></button>; })}
              {upcoming.map((r: any, i: number) => { const pos = UPCOMING_POSITIONS[i % UPCOMING_POSITIONS.length]; return <div key={r.id} className="absolute -translate-x-1/2 -translate-y-1/2 pointer-events-none z-10 opacity-75" style={pos}><div className="w-28 h-28 rounded-[42%] bg-white/55 border-4 border-white grid place-items-center backdrop-blur-sm"><Lock size={30} strokeWidth={3} className="text-ink-muted" /></div></div>; })}
            </>}
            ...
            {player && <motion.div data-testid="hero-sprite" className="absolute pointer-events-none z-40" initial={false} animate={{ left: `${hero.x}%`, top: `${hero.y}%` }} transition={{ duration: WALK_DURATION_S, ease: "easeInOut" }} style={{ translateX: "-50%", translateY: "-100%" }}><motion.div animate={{ y: [0, -3, 0, -3, 0] }} transition={{ duration: WALK_DURATION_S, ease: "easeInOut", repeat: 0 }} key={`${hero.x.toFixed(0)}-${hero.y.toFixed(0)}`}><ChibiAvatar config={player.avatar} size={74} /></motion.div></motion.div>}
          </div>
